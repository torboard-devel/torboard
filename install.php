...
if (file_exists($config['has_installed'])) {
	...
	switch ($version) {
		...
		default:
			...
			$page['body'] = '<p style="text-align:center">TORboard was unable to determine what version is currently installed.</p>';
			break;
		case VERSION:
			$page['title'] = 'Already installed';
			$page['body'] = '<p style="text-align:center">It appears that TORboard is already installed (' . $version . ') and there is nothing to upgrade! Delete <strong>' . $config['has_installed'] . '</strong> to reinstall.</p>';
			break;
	}			
...
if ($step == 0) {
	...
	$tests = array(
		...
		array(
			...
			'result' => is_writable('.'),
			'required' => true,
			'message' => 'TORboard does not have permission to create directories (boards) here. You will need to <code>chmod</code> (or operating system equivalent) appropriately.'
		),
		array(
			'category' => 'File permissions',
			...
			'required' => true,
			'message' => 'You must give TORboard permission to create (and write to) the <code>templates/cache</code> directory or performance will be drastically reduced.'
		),
		array(
			'category' => 'File permissions',
			...
			'required' => true,
			'message' => 'You must give TORboard permission to write to the <code>tmp/cache</code> directory.'
		),
		array(
			'category' => 'File permissions',
			...
			'required' => false,
			'message' => 'TORboard does not have permission to make changes to <code>inc/secrets.php</code>. To complete the installation, you will be asked to manually copy and paste code into the file instead.'
		),
		array(
			'category' => 'Misc',
...
';
	...
	$page['body'] = '<p style="text-align:center">Thank you for using TORboard. Please remember to report any bugs you discover. <a href="https://github.com/vichan-devel/vichan/wiki/Configuration-Basics">How do I edit the config files?</a></p>';
	
	if (!empty($sql_errors)) {
		$page['body'] .= '<div class="ban"><h2>SQL errors</h2><p>SQL errors were encountered when trying to install the database. This may be the result of using a database which is already occupied with a TORboard or vichan installation; if so, you can probably ignore this.</p><p>The errors encountered were:</p><ul>' . $sql_errors . '</ul><p><a href="?step=5">Ignore errors and complete installation.</a></p></div>';
	} else {
		$boards = listBoards();
		foreach ($boards as &$_board) {
			setupBoard($_board);
	...
	$page['title'] = 'Installation complete';
	$page['body'] = '<p style="text-align:center">Thank you for using TORboard. Please remember to report any bugs you discover.</p>';
